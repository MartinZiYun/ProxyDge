...
commits: ffb7199..HEAD # filled at delivery
---
...
2. **混合地址族欺骗路径**(字节级实证):当入站 PROXY 头内部 src/dst 地址族矛盾(唯一可穿透形态:声明 AF_INET6 但 dst 为 `::ffff:` 映射的 IPv4),当前管道会静默转换输出——v2 输出原样透传 AF_INET6+`::ffff:c0a8:101`,v1 输出会发文本 `PROXY TCP6 ... ::ffff:192.168.1.1`。下游可能把映射地址当作真实 IPv6 目标去连接,导致连接失败或误连——这比诚实告知"地址不可知"更恶劣。
...
| `legacy` | 跳过检测、零特判,由参数化后的底层编码器按库原生语义处理:v2 输出延续现状的 `To16()`/`To4()` 映射(AF_INET6 + `::ffff:` 透传,与本特性之前逐字节一致);v1 输出走 formatVersion1 原生分支——TCPv6 时 dst 经 `To16()` 以 `::ffff:` 文本发出,TCPv4 且目的不可转时返回 ErrInvalidAddress,gateway 走现有写头错误路径(记日志、关连接) |
